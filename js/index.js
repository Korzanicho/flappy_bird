...
const bird = {
	...
	update: function(){
		...
		if(state.current == state.getReady){
			...
			if(this.y + this.h/2 >= ch - fg.h){
				this.y = ch - fg.h - this.h/2;
				if(state.current == state.game){
...
const score = {
	...
	draw: function(){
		...
		if(state.current==state.game){
			ctx.lineWidth = 2;
			ctx.font = '35px Teko';
			ctx.fillText(this.value, cw/2, 50);
			ctx.strokeText(this.value, cw/2, 50);
